...
class GzGhtAliPay extends GzGhtPay
{
        ...
        $orderId, $body, $fee, $returnUrl, array $options = [])
    {
        ...
        $data      = $this
            ->buildGatewayRequestData($orderId, $fee, $body, '', $returnUrl, 'PUBLICALIPAY', $notifyUrl, null, $options);

        $this->logger->info('PAY ALIPAY request', $data);
    ...
    public function microPay($orderId, $title, $body, $fee, $authCode, array $options = [])
    {
        $data = $this->buildNativeOrMicroRequestData(
            'BACKSTAGE_PAY',
            $orderId, 'BACKSTAGEALIPAY',
            $authCode, $fee, $title, $body, $options['notify_url']);

        $this->logger->info('BACKSTAGE_PAY ALI request', $data);
                                     ...
                                     $timeExpire = 30, array $options = [])
    {
        ...
        $this->logger->info('FRONT_PAY ALI request', $data);
        $response = $this->post('/backStageEntry.do', $data);
